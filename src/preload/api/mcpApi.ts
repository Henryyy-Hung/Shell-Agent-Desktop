...
const mcpApi = {
  connectAll: (): Promise<void> => ipcRenderer.invoke(IpcChannel.Mcp_ConnectAll),
  disconnectAll: (): Promise<void> => ipcRenderer.invoke(IpcChannel.Mcp_DisconnectAll),
  restartAll: (): Promise<void> => ipcRenderer.invoke(IpcChannel.Mcp_RestartAll),
  ...
  getPrompt: (serverName: string, name: string, args: Record<string, any> = {}): Promise<any> =>
  ...
  callTool: (
    ...
    callId?: string
  ): Promise<any> => ipcRenderer.invoke(IpcChannel.Mcp_CallTool, serverName, name, args, callId),
