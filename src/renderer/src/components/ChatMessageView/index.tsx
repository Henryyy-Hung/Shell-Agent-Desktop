...
import {
  ...
  SessionMessages,
  SessionName
} from '@renderer/components/ChatMessageView/styles'
...
const sessionNameMap: Record<MessageOwnerEnumType, string> = {
  [MessageOwnerEnum.PUBLIC]: '系统提示词',
  [MessageOwnerEnum.USER]: '用户',
  [MessageOwnerEnum.AGENT_COMMON]: '通用助手',
  [MessageOwnerEnum.AGENT_PROJECT_MANAGER]: '项目经理',
  [MessageOwnerEnum.AGENT_FRONTLINE_ENGINEER]: '一线工程师',
  [MessageOwnerEnum.AGENT_SENIOR_ENGINEER]: '高级工程师',
  [MessageOwnerEnum.AGENT_ARCHIVE_ADMIN]: '档案管理员'
}
...
const ChatMessageView: React.FC<MessageBlockProps> = ({ message }): React.JSX.Element => {
  return (
    <MessageContainer>
      <SessionInfo>
        <Avatar />
        <Desc>
          <SessionName>{sessionNameMap[message.owner]}</SessionName>
          <SessionCreationTime>
