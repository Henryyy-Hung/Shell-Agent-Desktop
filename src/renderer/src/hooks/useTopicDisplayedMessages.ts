...
function useTopicDisplayedMessages({ topicId }: { topicId: string | null }): ChatMessageVO[] {
  const selectedTopic = useSelector((state: RootState) => selectTopicById(state, topicId || ''))
  const EMPTY_MESSAGES: ChatMessage[] = []

  const messages = useSelector((state: RootState) =>
    selectedTopic ? selectMessagesByIds(state, selectedTopic.messageIds) : EMPTY_MESSAGES
  )
